...
def mostrar(parent):
    ...
    def acao_remarcar(id_consulta):
        ...
        lbl_topo = ctk.CTkLabel(
            ...
            text="Escolha a Nova Data e Horário",
    ...
    def abrir_detalhes_paciente(paciente_id):
        print(f"Abrindo detalhes do paciente ID: {paciente_id}")
    ...
    ctk.CTkLabel(frame_header, text="Paciente", font=("Segoe UI", 12, "bold"), text_color="#a0a0a5").grid(
        row=0, column=0, sticky="w", padx=15, pady=5
    )
    ctk.CTkLabel(frame_header, text="Horário", font=("Segoe UI", 12, "bold"), text_color="#a0a0a5").grid(
        row=0, column=1, sticky="w", padx=5, pady=5
    )
    ctk.CTkLabel(frame_header, text="Tratamento", font=("Segoe UI", 12, "bold"), text_color="#a0a0a5").grid(
        row=0, column=2, sticky="w", padx=5, pady=5
    )
    ctk.CTkLabel(frame_header, text="Ações", font=("Segoe UI", 12, "bold"), text_color="#a0a0a5").grid(
        row=0, column=3, sticky="e", padx=25, pady=5
    )
            ...
            if faltantes:
                ...
                for faltante in faltantes:
                    ...
                    lbl_nome = ctk.CTkLabel(
                        linha_frame, text=faltante["nome"], font=("Segoe UI", 13, "bold"), text_color="#ffffff"
                    )
                    ...
                    lbl_data = ctk.CTkLabel(
                        linha_frame, text=horario_texto, font=("Segoe UI", 12), text_color="#cfd0d4"
                    )
                    lbl_data.grid(row=0, column=1, sticky="w", padx=5, pady=10)

                    # Coluna 2: Tratamento
                    lbl_trat = ctk.CTkLabel(
                        linha_frame, text=faltante["tratamento"], font=("Segoe UI", 12), text_color="#a0a0a5"
                    )
                    lbl_trat.grid(row=0, column=2, sticky="w", padx=5, pady=10)

                    # Vinculando o Duplo Clique
                    linha_frame.bind(
                        "<Double-Button-1>", lambda event, p_id=faltante["id_paciente"]: abrir_detalhes_paciente(p_id)
                    )
                    lbl_nome.bind(
                        "<Double-Button-1>", lambda event, p_id=faltante["id_paciente"]: abrir_detalhes_paciente(p_id)
                    )
                ...
                linha_frame = ctk.CTkFrame(
                    tabela_scroll, height=50, corner_radius=6, border_width=1, border_color="#242528"
                )
                linha_frame.pack(fill="x", pady=4, padx=5)
                linha_frame.pack_propagate(False)

                Linha_Compareceu = ctk.CTkLabel(
                    linha_frame, text="Ninguém faltou nesse dia", font=("Segoe UI", 14, "bold"), text_color="#a0a0a5"
                )
