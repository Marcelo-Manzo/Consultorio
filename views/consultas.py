...
from database.consultas import criar_consulta, listar_consultas_data, listar_consultas_paciente, listar_tratamentos
...
def mostrar(parent):
    ...
    frame_formulario = ctk.CTkFrame(
        container_principal, fg_color="#141517", border_width=1, border_color="#242528", corner_radius=10
    )
    ...
    lbl_secao_form = ctk.CTkLabel(
        frame_formulario, text="Novo Agendamento", font=("Segoe UI", 16, "bold"), text_color="#ffffff"
    )
    ...
    nome_busca_entry = ctk.CTkEntry(
        frame_busca_linha, placeholder_text="Nome do paciente", fg_color="#2b2b2b", height=35
    )
    ...
    tratamento_dropdown = ctk.CTkComboBox(
        frame_formulario, values=tratamentos_lista, width=280, height=35, fg_color="#2b2b2b", button_color="#3a3a3a"
    )
    ...
    data_entry = ctk.CTkEntry(
        frame_formulario, width=280, height=35, placeholder_text="Data (DD/MM/AAAA)", fg_color="#2b2b2b"
    )
    ...
    horario_dropdown = ctk.CTkComboBox(
        frame_formulario, values=horarios_padrao, width=280, height=35, fg_color="#2b2b2b", button_color="#3a3a3a"
    )
    ...
    valor_entry = ctk.CTkEntry(
        frame_formulario, width=280, height=35, placeholder_text="Valor (ex: 150.00)", fg_color="#2b2b2b"
    )
    ...
    def agendar():
        ...
        try:
            ...
            metodo_dropdown.set("Método de pagamento")
            # resposta
            resultado_label.configure(text="✓ Consulta agendada com sucesso!", text_color="#4ade80")
    ...
    lista_label = ctk.CTkLabel(
        frame_historico, text="Histórico do Paciente", font=("Segoe UI", 16, "bold"), text_color="#a0a0a5"
    )
    ...
    def atualizar_lista():
        ...
        for c in consultas:
            ...
            card_historico = ctk.CTkFrame(
                lista_frame, fg_color="#212225", border_width=1, border_color="#3a3a3a", corner_radius=8
            )
            card_historico.pack(fill="x", padx=5, pady=5)

            ctk.CTkLabel(card_historico, text=texto, justify="left", font=("Segoe UI", 12), text_color="#cfd0d4").pack(
